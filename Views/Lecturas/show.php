...
<?php $meses = array(
    ...
    <button type="button" class="btn btn-primary" style="margin-left:1%;" onclick="location.href='?controller=evaluacionV&action=registerTec'">
        <span class="p-1">
            <i class="fas fa-plus"></i>
        </span>Nuevo
    </button>
    <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><span class="p-1"><i class="fas fa-filter"></i></span>Buscar</button>
    <button type="button" class="btn btn-success" style="margin:1%;" onclick="location.href='?controller=evaluacionV&action=showLecturaEMP'"><span class="p-1"><i class="fas fa-redo-alt"></i></span>Limpiar</button>
   
   
    
        <?php foreach($GET_MES_ID as $listActivo){ if($listActivo->getEstado()=='si'){?>
            <h4 class="" style="text-align:center" id="meactivo">Mes Activo Actual</h4>
            <h5 style="text-align:center" value="<?php echo $listActivo->getId(); ?>" selected><?php echo $listActivo->getAnnio().' - '.$meses[$listActivo->getMes()]; ?></h5>
        <?php }else{ ?>
            ...
            <div class="modal-body">
                <ul class="list-group">
                    <li class="list-group-item" style="border:none;">
                        <form class="form-inline" action="?controller=evaluacionV&action=buscarExtendidaTec" method="post" name="ola" id="oplm">
                        <div class="">
                                <div class="">
                            ...
                            <div class="form-group mt-3">
                                <div class="col-xs-4">
                                    <button type="submit" class="btn btn-primary" ><span><i class="fas fa-search" ></i></span> Buscar</button>
                                    <button type="button" class="btn btn-secondary" style="margin-left:1%;" data-bs-dismiss="modal" aria-label="Close"><span><i class="fas fa-times"></i></span>Cerrar</button>
                                </div>
                            </div>
                        </form>
                    </li>
            ...
            <tbody>
                <?php
					foreach ($paginationUtils->getListaElementos() as $evaluacion)  {
                        ...
                        $catalogos=Catalogos::getByIdM($evaluacion->getEstadoMedidor())
						?>
                <tr class="accordion">
                    <td><?php echo $proyecto->getNombre(); ?></td>
                    <td><?php echo $etapa->getNombre(); ?></td>
                    <td><?php echo $poligono->getNombre();?></td>
                    <td><?php echo $lote->getNumeroLote();?></td>
                    <td><?php echo $lote->getNumeroSerieMedidor();?></td>
                    <td><?php echo $evaluacion->getLecturaFinal();?></td>
                    <td><?php echo $catalogos->getDescripcion(); ?></td>
                    <td><?php echo $evaluacion->getEstadoVivienda();?></td>
                    <td><?php echo $evaluacion->getTipoVivienda();?></td>
                    <td id="hide" class="notog"><button type="button" class="btn btn-danger"
                            onclick="location.href='?controller=evaluacionV&action=deleteTec&id=<?php echo $evaluacion->getId(); ?>'"><span
                    ...
                    <td id="hide" class="notog"><button  type="button" class="btn btn-primary"
                            onclick="location.href='?controller=evaluacionV&action=showUpdateTec&id=<?php echo $evaluacion->getId(); ?>'"><i
                ...
                <tr class="rowContent" style="border-style:hidden;">
                    <td class="hl" style="width:130px;overflow: auto;"><button type="button" class="btn btn-danger"
                            onclick="location.href='?controller=evaluacionV&action=deleteTec&id=<?php echo $evaluacion->getId(); ?>'"><span
                                class="p-1"><i class="bi bi-trash-fill"></i></span>Eliminar</button></td>
                    <td class="hl" style="width:130px;overflow: auto;"><button disabled="true" type="button" class="btn btn-primary"
                            onclick="location.href='?controller=evaluacionV&action=showUpdateTec&id=<?php echo $evaluacion->getId(); ?>'">
...
<nav aria-label="Page navigation " id="lol1">
    <ul class="pagination justify-content-center" id="lol1">
        ...
        <li class="page-item">
            <a class="page-link"
                href="?controller=evaluacionV&V&action=showLecturaEMP&boon=<?php echo $paginationUtils->getAnterior(); ?>">Previous</a>
        </li>
        <?php } else { ?>
        ...
        <li class="page-item"><a class="page-link"
                href="?controller=evaluacionV&V&action=showLecturaEMP&boon=<?php echo $paginationUtils->getPrimero(); ?>"><?php echo $paginationUtils->getPrimero(); ?></a>
        </li>
        <li class="page-item disabled"><a class="page-link" href="#">...</a></li>
        <?php }
		for ($i=$paginationUtils->getInicioPaginacion();$i<=$paginationUtils->getNumeroCiclos();$i++){ ?>
        <?php if($_GET['boton']==$i) {?>
        <li class="page-item active"><a class="page-link" href="#"><?php echo $i; ?><span class="sr-only">(página
                    actual)</span></a></li>
        <?php } else { ?>
        <li class="page-item"><a class="page-link"
                href="?controller=evaluacionV&V&action=showLecturaEMP&boon=<?php echo $i ?>"><?php echo $i; ?></a></li>
        <?php } ?>
        <?php }
		if($paginationUtils->getVisibleUltimo()){ ?>
        <li class="page-item disabled"><a class="page-link" href="#">...</a></li>
        <li class="page-item"><a class="page-link"
                href="?controller=evaluacionV&V&action=showLecturaEMP&boon=<?php echo $paginationUtils->getUltimo(); ?>"><?php echo $paginationUtils->getUltimo(); ?></a>
        </li>
        <?php } ?>
        <?php if($paginationUtils->getActivoSiguiente()){ ?>
        <li class="page-item"><a class="page-link"
                href="?controller=evaluacionV&V&action=showLecturaEMP&boon=<?php echo $paginationUtils->getSiguiente(); ?>">next</a>
        </li>
        <?php } else {?>
