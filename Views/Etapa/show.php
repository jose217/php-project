...
<div class="container">
    <h1 class="mt-3">Lista de Etapas</h1>
    <button type="button" class="btn btn-primary"
        onclick="location.href='?controller=etapa&action=register'">
        <span class="p-1">
    ...
    <button type="button" class="btn btn-success"
        onclick="location.href='?controller=etapa&action=show'"><span class="p-1"><i
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Buscar Etapa</h5>
                </div>
                <div class="modal-body">
                    <ul class="list-group">
                        <li class="list-group-item" style="border:none;">
                            <form class="form-inline" action="?controller=etapa&action=buscar" method="post" name="ola"
                                id="oplm">
                                <div class="">
    ...
    <div style="margin-top:1%;">
        <div class="table-responsive">
            ...
            <table id="Table" class="table research" style="width:100%;">
                ...
                <tbody>
                    <?php
                    foreach ($paginationUtils->getListaElementos() as $etapa) {
                        $proyecto = Residencia::getById($etapa->getProyectoId());
                        ?>
                        <tr class="accordion">
                            <td>
                            ...
                            <td id="hide" class="notog"><button type="button" class="btn btn-danger"
                                    onclick="location.href='?controller=etapa&action=delete&id=<?php echo $etapa->getId(); ?>'"><span
                            ...
                            <td id="hide" class="notog"><button type="button" class="btn btn-primary"
                                    onclick="location.href='?controller=etapa&action=showupdate&id=<?php echo $etapa->getId(); ?>'"><span
    ...
    <nav aria-label="Page navigation " id="lol1">
        <ul class="pagination justify-content-center" id="lol1">
            <?php if ($paginationUtils->getActivoAnterior()) { ?>
                <li class="page-item">
                    <a class="page-link"
                        href="?controller=etapa&action=show&boton=<?php echo $paginationUtils->getAnterior(); ?>">Previous</a>
                </li>
            <?php } else { ?>
            ...
            <?php if ($paginationUtils->getVisiblePrimero()) { ?>
                <li class="page-item"><a class="page-link"
                        href="?controller=etapa&action=show&boton=<?php echo $paginationUtils->getPrimero(); ?>">
            ...
            for ($i = $paginationUtils->getInicioPaginacion(); $i <= $paginationUtils->getNumeroCiclos(); $i++) { ?>
                ...
                <?php } else { ?>
                    <li class="page-item"><a class="page-link" href="?controller=etapa&action=show&boton=<?php echo $i ?>">
                            <?php echo $i; ?>
                        </a></li>
                <?php } ?>
            <?php }
            if ($paginationUtils->getVisibleUltimo()) { ?>
                <li class="page-item disabled"><a class="page-link" href="#">...</a></li>
                <li class="page-item"><a class="page-link"
                        href="?controller=etapa&action=show&boton=<?php echo $paginationUtils->getUltimo(); ?>">
            ...
            <?php if ($paginationUtils->getActivoSiguiente()) { ?>
                <li class="page-item"><a class="page-link"
                        href="?controller=etapa&action=show&boton=<?php echo $paginationUtils->getSiguiente(); ?>">next</a>
                </li>
            <?php } else { ?>
