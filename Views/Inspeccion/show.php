...
<div class="container">
    <h1 class="mt-3">Inspección</h1>

    <button type="button" class="btn btn-primary" 
        onclick="location.href='?controller=Inspeccion&action=showSolicitudesByClientActive'">
        <span class="p-1">
    ...
    <button disabled type="button" class="btn btn-success"
        onclick="location.href='?controller=Inspeccion&action=show'">
        <span class="p-1">
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Buscar Inspecciones</h5>
                </div>
                <div class="modal-body">
                    <ul class="list-group">
                        <li class="list-group-item" style="border:none;">
                            <form class="form-inline" action="?controller=Inspeccion&action=buscar" method="post"
                                name="ola" id="oplm">
                                <div class="">
            ...
            <table id="Table" class="table research" style="width:100%;">
                ...
                <tbody>
                    ...
                    foreach ($paginationUtils->getListaElementos() as $row) {
                        $datosSolicitud = InspeccionSolicitud::getById($row->getIvSolicitudId());
                        $datosLote = Lote::getById($datosSolicitud->getLoteId());
                        ?>
                        <tr class="accordion">
                            <td><?php echo $row->getIvSolicitudId(); ?></td>
                            <td><?php echo $datosSolicitud->getCodCliente(); ?></td>
                            <td><?php echo $datosLote->getNumeroLote() ?></td>
                            <td><?php echo $row->getIvFechaVisita(); ?></td>
                            <td id="hide" class="notog"><button type="button" class="btn btn-danger"
                                    onclick="location.href='?controller=Inspeccion&action=delete&id=<?php echo $row->getId(); ?>'"><span
                                        class="p-1"><i class="bi bi-trash-fill"></i></span>Eliminar</button></td>
                            <td id="hide" class="notog"><button disabled type="button" class="btn btn-primary"
                                    onclick="location.href='?controller=Inspeccion&action=showupdate&id=<?php echo $row->getId(); ?>'"><span
                                        class="p-1"><i class="fas fa-sync-alt"></i></span>Editar</button></td>
                            <td id="hide" class="notog"><button type="button" class="btn btn-primary"
                                    onclick="location.href='?controller=Inspeccion&action=getDocumentInsId'"><span
                                        class="p-1"><i class="fas fa-print"></i></span>Generar Documento</button></td>
                        </tr>
    ...
    <nav aria-label="Page navigation " id="lol1">
        <ul class="pagination justify-content-center" id="lol1">
            <?php if ($paginationUtils->getActivoAnterior()) { ?>
                <li class="page-item">
                    <a class="page-link"
                        href="?controller=Inspeccion&action=show&boton=<?php echo $paginationUtils->getAnterior(); ?>">Previous</a>
                </li>
            <?php } else { ?>
            ...
            <?php if ($paginationUtils->getVisiblePrimero()) { ?>
                <li class="page-item"><a class="page-link"
                        href="?controller=Inspeccion&action=show&boton=<?php echo $paginationUtils->getPrimero(); ?>">
            ...
            for ($i = $paginationUtils->getInicioPaginacion(); $i <= $paginationUtils->getNumeroCiclos(); $i++) { ?>
                ...
                <?php } else { ?>
                    <li class="page-item"><a class="page-link" href="?controller=Inspeccion&action=show&boton=<?php echo $i ?>">
                            <?php echo $i; ?>
                        </a></li>
                <?php } ?>
            <?php }
            if ($paginationUtils->getVisibleUltimo()) { ?>
                <li class="page-item disabled"><a class="page-link" href="#">...</a></li>
                <li class="page-item"><a class="page-link"
                        href="?controller=Inspeccion&action=show&boton=<?php echo $paginationUtils->getUltimo(); ?>">
            ...
            <?php if ($paginationUtils->getActivoSiguiente()) { ?>
                <li class="page-item"><a class="page-link"
                        href="?controller=Inspeccion&action=show&boton=<?php echo $paginationUtils->getSiguiente(); ?>">next</a>
                </li>
            <?php } else { ?>
